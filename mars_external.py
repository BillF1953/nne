# CUDA_VISIBLE_DEVICES='' /home/tylerf/tensorflow/bin/python3.5 mar_external.py
...
class Mars():
    ...
    def simulate(self):
        ...
        y = 10 * np.sin(np.pi * x[:, 0] * x[:, 1]) - 20 * (x[:, 2] - 0.05) ** 2 + 10 * x[:, 3] + 5 * x[:,
                                                                                                     4] + np.random.normal(
            0, 10, self.sample_size)
        return (x, y)
...
if __name__ == '__main__':
    ...
    print("This took {} seconds to run with {} samples".format(datetime.now() - start_time, num_samp))
